...
describe('RepositoryService', () => {
    ...
    it('should be able to get owned repositories', () => {
        ...
        service.getOwnedRepositories(userId).subscribe(repositories => {
    ...
    it('should be able to get contributed repositories', () => {
        ...
        service.getContributedRepositories(userId).subscribe(repositories => {
            expect(repositories).toEqual(contributedRepositories);
        });
    ...
    it('should be able to update repo', () => {
        const dummyRepo: RepositorySave = {
            id: 1,
            name: 'repo name changed',
            url: 'repo url',
            description: 'repo description',
            ownerId: 1,
            contributors: [1]
        ...
        service.updateRepository(dummyRepo).subscribe(repository => {
            expect(repository).toBe(responseRepo);
        });
        ...
        httpMock.expectOne((req: HttpRequest<any>) => {
            const body: RepositorySave = req.body;
            return req.url === url && req.method === 'PATCH' && body === dummyRepo;
        }, 'PATCH to api/repositories with repository data in json format')
    ...
    it('shouldn\'t be able to update non existing repo', () => {
        const dummyRepo: RepositorySave = {
            id: 1,
            name: 'repo name changed',
            url: 'repo url',
        ...
        service.updateRepository(dummyRepo).subscribe(
            repo => {
                expect(repo).toBeFalsy();
            },
            err => {
